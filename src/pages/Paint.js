...
function Canvas({ id, onDelete, ws, color, brushSize, isEraser, onClear, onClick, chatroomId }) {
    ...
    const draw = useCallback((e) => {
        ...
        // 그리기 데이터 전송 시 chatroomId 포함
        if (ws && ws.readyState === WebSocket.OPEN) {
            const drawData = {
                type: 'draw',
                chatroomId: parseInt(chatroomId, 10),  // chatroomId 추가 및 숫자로 변환
                canvasId: id,
                x: coords.x,
                y: coords.y,
                lastX: lastX,
        ...
        setLastX(coords.x);
        setLastY(coords.y);
    }, [isDrawing, lastX, lastY, color, brushSize, isEraser, id, ws, chatroomId]);  // chatroomId를 의존성 배열에 추가
    ...
    const handleTouchMove = useCallback((e) => {
        if (e.touches.length === 2) {
            // 두 손가락 터치는 상위 컨테이너에서 처리하도록 이벤트 전파
            return;
        }
        
        // 두 손가락 모드거나 그리기 중이 아니면 무시
        if (isTwoFingerTouch || !isDrawing) return;
        ...
        setLastX(coords.x);
        setLastY(coords.y);
    }, [isDrawing, lastX, lastY, color, brushSize, isEraser, drawLine, isTwoFingerTouch, ws, id]);
